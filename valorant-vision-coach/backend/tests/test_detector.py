...
def _frame_with_red_dot(w, h, cx, cy, r=5):
    frame = np.zeros((h, w, 3), dtype=np.uint8)
    cv2.circle(frame, (cx, cy), r, (0, 0, 255), -1)  # filled red (BGR)
    return frame


def test_minimap_detector_finds_red_marker_in_roi():
    det = MinimapColorDetector(SETTINGS)
    w, h = 1280, 720
    rx, ry, rw, rh = SETTINGS.minimap_calibration().roi_pixels(w, h)
    cx, cy = rx + rw // 2, ry + rh // 2
    frame = _frame_with_red_dot(w, h, cx, cy)
...
def test_minimap_detector_ignores_marker_outside_roi():
    det = MinimapColorDetector(SETTINGS)
    w, h = 1280, 720
    # A red dot in the bottom-right, far outside the top-left minimap ROI.
    frame = _frame_with_red_dot(w, h, w - 30, h - 30)
